...
- **`pos_weight` configurable según tolerancia al riesgo**: valor por defecto `3.5` en `mlp_deteccion_tiempo_real/train.py`, elegido por mejor equilibrio precisión/recall (F1 67%, recall 85.4%, precisión 55.2%). Si la prioridad de negocio fuera minimizar al máximo los fallos no detectados, aunque cueste más falsas alarmas, cambiar a `pos_weight=6.08` sube el recall a ~93% a costa de la precisión.
